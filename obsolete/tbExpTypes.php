...
<script>
    ...
    dsExpTypes = new dhx.DataCollection();
    loadPermission()
    
    function loadGrid() {
        dsExpTypes.removeAll();
        dsExpTypes.load("tbExpTypesQy.php?t=grid").then(function(){
    ...
    function addEditUser(argument) {
        ...
        form.events.on("click", function(name,e){
            ...
            if ( name == 'send' ) {
                const config = {
                    header: "Expense types ",
                    text: "Confirm expense type creation?",
                    buttons: ["no", "yes"],
                    buttonsAlignment: "center"
                };     
                dhx.confirm(config).then(function(answer){
                    if (answer) {
                        const send = form.send("tbExpTypesWr.php?t=f", "POST").then(function(data){
//                            message = JSON.parse(data);
        ...
        dhx.ajax.get("tbExpTypesQy.php?t=r&id="+wSelected).then(function (data) {
            console.log(data);
    ...
    function loadPermission() {
        dhx.ajax.get("menuQy.php?t=per&p=tbExpTypes").then(function (data) {
            console.log(data);
            var obj = JSON.parse(data);
            console.log(obj);
            canRead = obj.canRead;
