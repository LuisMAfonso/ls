...
<script>
    ...
    dsRoles = new dhx.DataCollection();
    loadPermission()
    
    function loadGrid() {
        dsRoles.removeAll();
        dsRoles.load("tbRolesQy.php?t=grid").then(function(){
    ...
    tbGrid.events.on("click", function(id,e){
        ...
        if ( id == 'delete' ) {  
            wMessage = {
                header: "Delete ", text: "Confirm Role deletion?", buttons: ["no", "yes"], buttonsAlignment: "center",
            };   
            dhx.confirm(wMessage).then(function(answer){
                if (answer) {
                    console.log(answer);
                    dhx.ajax.get("tbRolesWr.php?t=del&r="+wSelected).then(function (data) {
                        wSelected = 0;
                        tbGrid.disable(['edit', 'delete','add']);
                        loadGrid();
                }).catch(function (err) {
    ...
    function addEditUser(argument) {
        ...
        form.events.on("click", function(name,e){
            ...
            if ( name == 'send' ) {
                const config = {
                    header: "Business role ",
                    text: "Confirm business activity creation?",
                    buttons: ["no", "yes"],
                    buttonsAlignment: "center"
                };     
                dhx.confirm(config).then(function(answer){
                    if (answer) {
                        const send = form.send("tbRolesWr.php?t=f", "POST").then(function(data){
//                            message = JSON.parse(data);
        ...
        dhx.ajax.get("tbRolesQy.php?t=r&id="+wSelected).then(function (data) {
            console.log(data);
    ...
    function loadPermission() {
        dhx.ajax.get("menuQy.php?t=per&p=tbRoles").then(function (data) {
            console.log(data);
            var obj = JSON.parse(data);
            console.log(obj);
            canRead = obj.canRead;
