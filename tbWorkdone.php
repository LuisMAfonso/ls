...
<script>
    ...
    dsWorkdone = new dhx.DataCollection();
    loadPermission()
    
    function loadGrid() {
        dsWorkdone.removeAll();
        dsWorkdone.load("tbWorkdoneQy.php?t=grid").then(function(){
    ...
    function addEditUser(argument) {
        ...
        form.events.on("click", function(name,e){
            ...
            if ( name == 'send' ) {
                const config = {
                    header: "Work done ",
                    text: "Confirm work done creation?",
                    buttons: ["no", "yes"],
                    buttonsAlignment: "center"
                };     
                dhx.confirm(config).then(function(answer){
                    if (answer) {
                        const send = form.send("tbWorkdoneWr.php?t=f", "POST").then(function(data){
//                            message = JSON.parse(data);
        ...
        dhx.ajax.get("tbWorkdoneQy.php?t=r&id="+wSelected).then(function (data) {
            console.log(data);
    ...
    function loadPermission() {
        dhx.ajax.get("menuQy.php?t=per&p=tbWorkdone").then(function (data) {
            console.log(data);
            var obj = JSON.parse(data);
            console.log(obj);
            canRead = obj.canRead;
