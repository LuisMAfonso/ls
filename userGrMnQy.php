...
//  $db->debug=1;
  ...
  $sql = "SELECT ug.UserID, us.UserName
          FROM UsersGroups ug
          INNER JOIN Users us on us.UserID = ug.UserID
          WHERE UGroupID = $g
  ...
  while(!$rows_emps->EOF) {
    ...
    if ( $firstReg == 1 ) $firstReg = 0;
    print('{');
    print(' "UserID": "'.trim($rows_emps->fields[0]).'", ');
    print(' "UserName": "'.trim($rows_emps->fields[1]).'" ');
    print('}');
    $rows_emps->MoveNext();
  }
  $rows_emps->Close();
