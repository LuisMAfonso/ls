...
<script>
    ...
    dsUsrGrp = new dhx.DataCollection();
    loadPermission()
    
    function loadGrid() {
        dsUsrGrp.removeAll();
        dsUsrGrp.load("userGrpsQy.php?t=grid").then(function(){
	...
	tbGrid.data.load("toolbarsQy.php?t=g_aed").then(function(){
    ...
    tbGrid.events.on("click", function(id,e){
        ...
        if ( id == 'delete' ) {  
            wMessage = {
                header: "Delete ", text: "Confirm User Group deletion?", buttons: ["no", "yes"], buttonsAlignment: "center",
            };   
            dhx.confirm(wMessage).then(function(answer){
                if (answer) {
                    console.log(answer);
                    dhx.ajax.get("userGrpsWr.php?t=del&r="+wSelected).then(function (data) {
                        wSelected = 0;
                        tbGrid.disable(['edit', 'delete','add']);
                        loadGrid();
                }).catch(function (err) {
    ...
    function addEditUser(argument) {
        ...
        form.events.on("click", function(name,e){
            ...
            if ( name == 'send' ) {
                const config = {
                    header: "Users groups",
                    text: "Confirm users group creation?",
                    buttons: ["no", "yes"],
                    buttonsAlignment: "center"
                };     
                dhx.confirm(config).then(function(answer){
                    if (answer) {
                        const send = form.send("userGrpsWr.php?t=f", "POST").then(function(data){
//                            message = JSON.parse(data);
        ...
        dhx.ajax.get("userGrpsQy.php?t=r&id="+wSelected).then(function (data) {
            console.log(data);
    ...
    function loadPermission() {
        dhx.ajax.get("menuQy.php?t=per&p=userGrps").then(function (data) {
            console.log(data);
            var obj = JSON.parse(data);
            console.log(obj);
            canRead = obj.canRead;
